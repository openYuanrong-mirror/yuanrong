...
def _sanitize_err(exc: BaseException) -> str:
    """Flatten an exception message for log lines (single line, no escapes).

    Exception messages may contain newlines/control characters that would
    break the ``[sandbox.<op>.exit] result=...`` structured format.
    """
...
class Sandbox:
    ...
    def restore_instance(self, checkpoint_id: str,
        ...
        try:
            ...
            _sandbox_trace_exit("restore", trace_id, request_id,
                                result=f"ERR:{_sanitize_err(_root)}",
                                cost_ms=int((time.monotonic() - start) * 1000))
            raise
