...
class TestTunnelClientLogging(unittest.TestCase):
    ...
    def test_upstream_http_error_status_is_visible_at_warning_level(self):
        """Connected tunnel should warn when upstream HTTP returns an error status."""
        tunnel_client = _load_tunnel_client_module()
        sent_messages = []
        ...
        client = tunnel_client.TunnelClient(upstream="http://127.0.0.1:28800")
        frame = tunnel_client.HttpReqFrame(
            id="req-404",
            method="GET",
            path="/missing",
            headers={},
        ...
        with patch.object(tunnel_client, "logger") as mock_logger:
            asyncio.run(client._handle_http(FakeWs(), FakeHttp(), frame))

        warning_templates = [call.args[0] for call in mock_logger.warning.call_args_list]
        expected_warning = (
            "Tunnel upstream HTTP request failed: id=%s method=%s path=%s status=%d"
        )
        self.assertEqual(warning_templates.count(expected_warning), 1)
        warning_call = next(
            call for call in mock_logger.warning.call_args_list
            if call.args[0] == expected_warning
        )
        self.assertEqual(warning_call.args[4], 404)
