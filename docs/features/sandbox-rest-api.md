...
| 面 | 入口 | 主要路径 | 用途 |
|----|------|----------|------|
| Frontend control API | `YR_SERVER_ADDRESS` | `/api/sandbox/v1/sandboxes...` | create/delete/invoke/stream 控制面 |
...
- binary 文件/目录传输走 `/direct/{safeID}/upload|download`，避免 JSON/base64 包装；当前 SDK 不再为大文件路径回退到 WS。
...
### 3.3 Stream WebSocket

URL：`GET /api/sandbox/v1/sandboxes/{sandboxID}/stream`

- WebSocket subprotocol：`sandbox.stream.v1`。
- 文本帧是 JSON control frame。
- 二进制帧格式：`[4B magic "YRS1"][2B op-id length][op-id][payload]`。
- 当前公开支持文件/目录上传下载：
  - `file.upload.start` → binary chunks → `file.upload.finish`
  - `file.download.start` → binary chunks → `file.download.done`
- `shell.open` / `shell.stdin` / `process.start` / `process.stdin` 会返回 `NotImplemented`；普通命令与 shell 会话走 HTTP invoke。
...
`sandbox_stream_*` 是 frontend stream handler 与 RRT RuntimeRPC 之间的内部方法，不是 direct `/invoke` 的公开 action。
...
| `YR_TLS` | `1` | `1/true/yes` → `https/wss` frontend；`0/false/no` → `http/ws` |
| `YR_GATEWAY_ADDRESS` | 空 | tunnel 和用户端口 URL 的 gateway；为空回退 `YR_SERVER_ADDRESS` |
| `YR_GATEWAY_TLS` | `0` | tunnel 外部连接使用 `wss` 或 `ws` |
| `YR_STREAM_ADDRESS` | 空 | stream WebSocket frontend host override |
| `YR_STREAM_TLS` | 空 | stream WebSocket TLS override；为空跟随 `YR_TLS` |
...
| stream | `{ws(s)}://{YR_STREAM_ADDRESS or YR_SERVER_ADDRESS}/api/sandbox/v1/sandboxes/{sandboxID}/stream` |
...
- process/shell 实时 WebSocket streaming 尚未公开；当前使用 HTTP submit/poll。
