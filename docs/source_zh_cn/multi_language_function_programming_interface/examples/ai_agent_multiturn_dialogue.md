...
本案例展示如何利用 openYuanrong 的 AI Agent 会话（AI Agent Session）能力，构建一个支持多轮交互的智能对话助手。通过 `waitForNotify` 和 `notify` 机制，函数可以在执行过程中挂起并等待用户输入，从而实现复杂的交互逻辑。
...
- **执行流挂起**：支持在函数内部通过 `waitForNotify` 挂起，等待同一会话的后续请求唤醒。
...
public class DialogueAgent {
    public Object handle(Context ctx, JsonObject input) {
        ...
        try {
            ...
            JsonObject nameInput = sess.waitForNotify(60000); // 挂起并等待输入，超时 60s
...
2. **网关**: 转发请求到绑定的实例
3. **函数实例**: 执行业务逻辑并进入 `sess.waitForNotify()`，执行线程挂起
